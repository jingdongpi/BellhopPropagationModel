...
const char* GetBellhopModelVersion() {
    return "BellhopPropagationModel v2.0.0 - Self-Contained Debian11";
}
...
#!/bin/bash
echo "🧪 测试自包含可执行文件: BellhopPropagationModel (Debian 11)"
...
echo "🎯 Debian 11 自包含可执行文件测试完成"
EOF

cat > dist/test_library.cpp << 'EOF'
// Debian 11 自包含动态库测试程序
#include "BellhopPropagationModelInterface.h"
#include <iostream>
#include <fstream>
#include <sstream>
...
int main() {
    std::cout << "🧪 测试自包含动态库: libBellhopPropagationModel.so (Debian 11)" << std::endl;
